<?php
/**
 * Configurar Webhook Direto
 * Configura os servidores WhatsApp para usar o novo webhook sem redirecionamento
 */

echo "🔧 CONFIGURANDO WEBHOOK DIRETO\n";
echo "==============================\n\n";
...
// Nova URL do webhook direto
$webhook_direto = 'http://212.85.11.238:8080/api/webhook_direto.php';

echo "📋 CONFIGURAÇÃO:\n";
echo "----------------\n";
echo "Webhook Direto: $webhook_direto\n";
echo "Servidores: " . implode(', ', array_keys($servers)) . "\n\n";

// 1. Testar conectividade do webhook direto
echo "1️⃣ TESTANDO CONECTIVIDADE DO WEBHOOK DIRETO\n";
echo "--------------------------------------------\n";
...
if ($http_code !== 200) {
    echo "❌ Webhook direto não está funcionando. Verifique se o arquivo foi criado corretamente.\n\n";
    exit;
}
...
foreach ($servers as $name => $server_url) {
    ...
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['url' => $webhook_direto]));
    curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json']);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_TIMEOUT, 10);
...
// 4. Instruções finais
echo "\n4️⃣ INSTRUÇÕES FINAIS\n";
echo "---------------------\n";

echo "✅ CONFIGURAÇÃO CONCLUÍDA!\n\n";

echo "📋 PRÓXIMOS PASSOS:\n";
echo "1. Teste enviando uma mensagem real via WhatsApp\n";
echo "2. Verifique se a mensagem aparece no chat do painel\n";
echo "3. Monitore os logs em: logs/webhook_direto_" . date('Y-m-d') . ".log\n";
echo "4. Se necessário, reinicie os servidores WhatsApp\n\n";

echo "🔧 COMANDOS ÚTEIS:\n";
echo "- Testar webhook: curl -X POST $webhook_direto -H 'Content-Type: application/json' -d '{\"event\":\"test\"}'\n";
echo "- Verificar status: curl $servers[default]/status\n";
echo "- Verificar logs: tail -f logs/webhook_direto_" . date('Y-m-d') . ".log\n\n";
