<?php
/**
 * Corrigir Mensagens Problemáticas
...
try {
    ...
    function calcularHorarioEnvioManual($estrategia) {
        switch ($estrategia) {
            case 'vencendo_hoje':
                return date('Y-m-d 09:00:00'); // Enviar hoje às 9h
            case 'vencidas':
                return date('Y-m-d 10:00:00'); // Enviar hoje às 10h
            case 'a_vencer':
                return date('Y-m-d 11:00:00'); // Enviar hoje às 11h
            default:
    ...
    function agendarMensagemUnica($cliente_id, $mensagem, $horario_envio, $prioridade, $mysqli) {
        ...
        $sql_delete = "DELETE FROM mensagens_agendadas 
                       WHERE cliente_id = ? AND status = 'agendada' AND data_agendada > NOW()";
        ...
        $sql_insert = "INSERT INTO mensagens_agendadas 
                       ...
                       VALUES (?, ?, 'cobranca_completa', ?, ?, 'agendada', NOW())";
        $stmt_insert = $mysqli->prepare($sql_insert);
        $stmt_insert->bind_param("iss", $cliente_id, $mensagem, $prioridade, $horario_envio);
    ...
    foreach ($clientes_problematicos as $cliente) {
        ...
        try {
            ...
            if ($sucesso) {
                ...
                throw new Exception("Erro ao agendar nova mensagem");
            }
