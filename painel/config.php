...
if (php_sapi_name() === 'cli') {
    ...
    // Se estiver via web, usar detecção melhorada
    $is_local = (
...
// Forçar ambiente local se estiver no XAMPP (desenvolvimento)
...
if (DEBUG_MODE) {
    ...
    error_log("[CONFIG] Ambiente detectado: {$env_info}{$port_info}{$db_info} | Host: " . ($_SERVER['SERVER_NAME'] ?? 'CLI'));
}

/**
 * IMPORTANTE: A chave da API do Asaas agora é armazenada exclusivamente no banco de dados
 * na tabela 'configuracoes' com chave = 'asaas_api_key'
 * Use api/get_asaas_config.php para recuperar a chave ativa
 */
?> 