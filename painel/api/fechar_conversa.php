<?php
/**
 * API para Fechamento Manual de Conversas
 * 
 * Endpoint: POST /api/fechar_conversa.php
 * Parâmetros: cliente_id (obrigatório)
 * Retorna: JSON com status da operação
 * 
 * IMPORTANTE: NÃO envia notificação WhatsApp ao fechar conversa
 */
...
try {
    ...
    // Marcar todas as mensagens da conversa como fechada
    $stmt = $mysqli->prepare("UPDATE mensagens_comunicacao SET status_conversa = 'fechada' WHERE cliente_id = ?");
    $stmt->bind_param('i', $cliente_id);
    ...
    if ($stmt->execute()) {
        ...
        $log_sql = "INSERT INTO mensagens_comunicacao (canal_id, cliente_id, mensagem, tipo, data_hora, direcao, status, status_conversa) 
                    VALUES (1, ?, 'Conversa fechada manualmente por $usuario', 'sistema', ?, 'enviado', 'enviado', 'fechada')";
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Conversa fechada com sucesso (sem notificação)',
        ...
        throw new Exception("Erro ao fechar conversa: " . $stmt->error);
    }
