<?php
/**
 * Webhook Fix - Solução Alternativa
 * Endpoint webhook sem redirecionamento usando nome de arquivo diferente
 */
...
// Salvar log
$log_file = '../logs/webhook_fix_' . date('Y-m-d') . '.log';
$log_data = date('Y-m-d H:i:s') . " [{$ambiente}] - " . $input . "\n";
file_put_contents($log_file, $log_data, FILE_APPEND);

// Log de debug
if (DEBUG_MODE) {
    error_log("[WEBHOOK FIX {$ambiente}] Requisição recebida");
    error_log("[WEBHOOK FIX {$ambiente}] Dados: " . $input);
}
...
if (isset($data['event']) && $data['event'] === 'onmessage') {
    ...
    if (DEBUG_MODE) {
        error_log("[WEBHOOK FIX {$ambiente}] Processando mensagem de: $numero");
    }
    ...
    if ($result && $result->num_rows > 0) {
        ...
        if (DEBUG_MODE) {
            error_log("[WEBHOOK FIX {$ambiente}] Cliente encontrado: {$cliente['nome']} (ID: $cliente_id)");
        }
    }
    ...
    if (!$cliente_id) {
        if (DEBUG_MODE) {
            error_log("[WEBHOOK FIX {$ambiente}] Cliente não encontrado, criando cadastro automático...");
        }
        ...
        if ($mysqli->query($sql_criar)) {
            ...
            if (DEBUG_MODE) {
                error_log("[WEBHOOK FIX {$ambiente}] ✅ Cliente criado automaticamente - ID: $cliente_id");
            }
        } else {
            error_log("[WEBHOOK FIX {$ambiente}] ❌ Erro ao criar cliente: " . $mysqli->error);
        }
    }
    ...
    if ($mysqli->query($sql)) {
        ...
        if (DEBUG_MODE) {
            error_log("[WEBHOOK FIX {$ambiente}] ✅ Mensagem salva - ID: $mensagem_id");
        }
        ...
        error_log("[WEBHOOK FIX {$ambiente}] ❌ Erro ao salvar mensagem: " . $mysqli->error);
    }
    ...
    if ($texto) {
        ...
        if (DEBUG_MODE) {
            error_log("[WEBHOOK FIX {$ambiente}] Enviando resposta via: $api_url");
        }
        ...
        if ($http_code === 200) {
            ...
            if ($api_result && isset($api_result["success"]) && $api_result["success"]) {
                if (DEBUG_MODE) {
                    error_log("[WEBHOOK FIX {$ambiente}] ✅ Resposta automática enviada");
                }
                ...
                error_log("[WEBHOOK FIX {$ambiente}] ❌ Erro ao enviar resposta: " . $api_response);
            }
        } else {
            error_log("[WEBHOOK FIX {$ambiente}] ❌ Erro HTTP ao enviar resposta: $http_code");
        }
    }
}
...
$response = [
    ...
    'webhook_type' => 'fix',
    'message' => 'Webhook fix processado com sucesso'
];
